(ns hp_vos3.main
  "Kotodama WASM entrypoint for the Hp_vos3 clean-room actor (L4) — Clojure port.
  ...
  facts (`hp_vos3.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "hp_vos3")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"hp_vos3.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "hp_vos3-compat" :tier tier :entities entities} 200])
...
(defn start! [] :hp_vos3-compat/ready)
